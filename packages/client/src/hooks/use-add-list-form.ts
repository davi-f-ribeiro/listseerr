...
import {
  ...
  getTraktChartPeriodDisplayName,
  StevenLuVariants,
  DEFAULT_STEVENLU_VARIANT,
  stevenLuVariantUrl,
  type StevenLuVariant,
...
const initialState: AddListFormState = {
  ...
  selectedPeriod: DEFAULT_TRAKT_CHART_PERIOD,
  seerrUserIdOverride: '',
  anilistUsername: '',
  anilistUsernameError: null,
  anilistStatus: AnilistStatusValues.PLANNING,
  stevenLuVariant: DEFAULT_STEVENLU_VARIANT,
...
export function useAddListForm() {
  ...
  // Build the traktChart list name, appending the period for period-based charts
  const buildTraktChartName = () => {
    const chartLabel = getTraktChartDisplayName(state.selectedChartType);
    const mediaLabel = state.selectedMediaType === TraktMediaTypeValues.MOVIES ? 'Movies' : 'Shows';
    const base = `${chartLabel} ${mediaLabel} ${getProviderDisplayName(state.provider)}`;
    return chartTypeNeedsPeriod(state.selectedChartType)
      ? `${base} (${getTraktChartPeriodDisplayName(state.selectedPeriod)})`
      : base;
  };

  // StevenLu list name: provider + the selected variant's label
  const buildStevenLuName = () =>
    `${getProviderDisplayName(state.provider)} ${StevenLuVariants[state.stevenLuVariant].label}`;

  // Auto-generate name for traktChart, stevenlu, and anilist
  const autoGeneratedName = (() => {
    if (isTraktChart(state.provider)) {
      return buildTraktChartName();
    } else if (isStevenLu(state.provider)) {
      return buildStevenLuName();
  ...
  const handlePeriodChange = (value: TraktChartPeriod) => {
    dispatch({ type: 'SET_FIELD', field: 'selectedPeriod', value });
  };
  ...
  const handleStevenLuVariantChange = (value: StevenLuVariant) => {
    dispatch({ type: 'SET_FIELD', field: 'stevenLuVariant', value });
  };
  ...
  const handleAdd = () => {
    ...
    // Period charts carry a period segment; others must not (Trakt 404s).
    if (isTraktChart(state.provider)) {
      const periodPath = chartTypeNeedsPeriod(state.selectedChartType)
        ? `/${state.selectedPeriod}`
        : '';
      finalUrl = `https://trakt.tv/${state.selectedMediaType}/${state.selectedChartType}${periodPath}`;
    } else if (isStevenLu(state.provider)) {
      finalUrl = stevenLuVariantUrl(state.stevenLuVariant);
